...
public class WeaponConfig extends ObjectType {
    ...
    public WeaponConfig(IObjectSpec objectSpecification) {
        ...
        s12k = writer.writeObject(specification -> new WeaponConfiguration(specification, 2.0F, 220, 1, 0.014F, 0.12F), "S12k");
        akm = writer.writeObject(specification -> new WeaponConfiguration(specification, 6.0F, 330, 4, 0.004F, 0.06F), "AKM");
        hk416 = writer.writeObject(specification -> new WeaponConfiguration(specification, 5.0F, 380, 6, 0.006F, 0.07F), "HK 416");
        ...
        vss = writer.writeObject(specification -> new WeaponConfiguration(specification, 6.0F, 140, 2, 0.008F, 0.08F), "VSS");
        ...
        grenadeLauncher = writer.writeObject(specification -> new SimpleWeaponConfiguration(specification, 1.6f, 0.02F, 0.12F), "Grenade Launcher");
        rocketLauncher = writer.writeObject(specification -> new SimpleWeaponConfiguration(specification, 2.0f, 0.02F, 0.13F), "Rocket Launcher");
    }
}
