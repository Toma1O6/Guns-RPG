...
public class DebuffCondition extends AbstractDefaultCondition {
    ...
    public Condition copy() {
        return new DebuffCondition(this.getDefaultFailureResponse(), this.debuff, this.status);
    }
    ...
    private ResponseType handleResponse(DeathEvent event, Quest quest) {
        ...
        if (Condition.checkIfEntityIsPartyMember(origin, quest.getParty())) {
            ...
            return PlayerData.get(player).map(data -> {
                ...
                boolean hasDebuff = debuffs.hasDebuff(debuff);
                return hasDebuff != status ? this.getDefaultFailureResponse() : ResponseType.OK;
